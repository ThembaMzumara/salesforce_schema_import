...
function runPythonScriptWithVenv(scriptPath: string, inputCsv: string) {
  try {
    ...
    if (result.error) {
      throw new Error(
        `Failed to execute Python script: ${result.error.message}`,
      );
    }
...
program
  ...
  .description(
    "Import metadata from a CSV file and process it into a structured format",
  )
...
program
  ...
  .description(
    "Generate Salesforce Object Query Language (SOQL) queries based on provided metadata JSON",
  )
...
program
  ...
  .description(
    "Generate Salesforce CLI commands based on metadata JSON for schema migration",
  )
  .argument("<jsonFile>", "Path to the JSON file containing parsed metadata")
  .action(async (jsonFile: string) => {
    try {
